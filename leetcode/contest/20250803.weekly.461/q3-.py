...
class Solution:
    def leetcode(self, s: str, order: List[int], k: int) -> int:
        count = 0
        ll = len(order)
        ...
        len_replaced = 0
        for ii in range(ll):
            #print(ii, before, order[ii], after, replaced, count)
            if len_replaced > 0:
                if order[ii] > replaced[-1]:
                    before = replaced[-1]
                    after = ll
                    replaced.append(order[ii])
                elif order[ii] < replaced[0]:
                    before = -1
                    after = replaced[0]
                    replaced = [order[ii]] + replaced
                else:
                    for jj in range(1,len_replaced):
                        if order[ii] < replaced[jj]:
                            before = replaced[jj-1]
                            after = replaced[jj]
                            replaced = replaced[0:jj] + [order[ii]] + replaced[jj:]
                            break
            else:
                replaced = [order[ii]]
            len_replaced += 1
            count += (order[ii]-before)*(after-order[ii])
            if count >= k:
                return ii
