...
class TaskManager:
    ...
    def add(self, userId: int, taskId: int, priority: int) -> None:
        ...
        #print("add", self.t, self.p)

    def edit(self, taskId: int, newPriority: int) -> None:
        self.p[taskId] = newPriority
        heapq.heappush(self.t, (-newPriority, -taskId))
        #print("edit", self.t, self.p)
        

    def rmv(self, taskId: int) -> None:
        self.p[taskId] = -1
        heapq.heappush(self.t, (1, -taskId))
        #print("remove", self.t, self.p)
        

    def execTop(self) -> int:
        while self.t:
            p, t = heapq.heappop(self.t)
            #print(self.t, self.p, p, t)
            if p > 0:
                return -1
            if -p == self.p[-t]:
                #print(p, self.u[-t])
